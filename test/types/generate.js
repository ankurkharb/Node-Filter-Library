...
async function writeParity() {
  const lines = [
    ...
    "import type * as T from 'node-query-filter';",
    '',
...
const DOC_GLOBALS = {
  ...
  filtering: "import('node-query-filter').Filtering",
  filterSet: "import('node-query-filter').FilterSet",
  UserFilterSet: "import('node-query-filter').FilterSet",
  STATUS_CHOICES: "import('node-query-filter').Choices",
  TAG_CHOICES: "import('node-query-filter').Choices",
  app: 'any',
  req: 'any',
  res: 'any',
  next: 'any',
  ...
  for (const file of files.filter((f) => f.endsWith('.md'))) {
    ...
    for (const [index, [, block]] of [...markdown.matchAll(/^```js\n([\s\S]*?)^```/gm)].entries()) {
      ...
      const source = [
        ...
        "import * as lib from 'node-query-filter';",
        imports.length > 0 ? `import { ${imports.join(', ')} } from 'node-query-filter';` : '',
